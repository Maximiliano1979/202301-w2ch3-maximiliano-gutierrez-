...
let cartonBingo = []; 
let numbersBasketBingo = [];
let namePlayer = [];
let counterRounds = 0;
let points = [107, 93, 80, "https://www.youtube.com/watch?v=x8zFL-0rBAw&ab_channel=DiditDeelon", 0];
const players = [
  {name: "Juan Carlos I",
   puntuacion: 93,},
  ...
  {name: "Carmen de Mairena",
   puntuacion: 95},
  {name: "Miguel 'alias El Quijote' Cervantes",
   puntuacion: 78},
];
...
const letStartTheGame = () => {
  function welcomeAndUser() {
      do {
        screenWelcomeUser = String(window.prompt("Bienvenido al BINGO ISDI, dinos tu nombre y apellido", "Nombre del jugador"));
          if (Number(screenWelcomeUser) || screenWelcomeUser === "Nombre del jugador" || screenWelcomeUser === "" || screenWelcomeUser === " " || screenWelcomeUser === "null") {
                  window.alert ("Necesito saber al menos tu nombre");
          } else {
              namePlayer.push(screenWelcomeUser);
              console.log(`%cEl jugador actual es: ` + namePlayer, "color:yellow; background-color:blue");
          };
...
/*     for (let i = 0; i < 5; i++) {
  ...
  function repeated(elements){
    return elements.some(function(element,position) { return elements.indexOf(element,position+1)!==-1 });
  }; 
  ...
  function doYouLikeCard () {
      let cardYesOrNot = confirm(`${namePlayer} ¿te gustaría jugar con este cartón?: (pulsa Cancelar para generar uno nuevo)
  ...
  function checkInBingoCard(num) {
    for(let i = 0; i < cartonBingo.length; i++) {
      ...
      if(isRow) {
        ...
        cartonBingo[i] = cartonBingo[i].map(lineNum => lineNum === num ? "X" : lineNum);
        isRow = isFullRow(cartonBingo[i]);
        if(isRow) {
          alert(`${namePlayer} has completado la linea ${i + 1} en el turno número ${counterRounds}`);
        };
      };
    };
  ...
  function playBingo() {
    for (num of numbersBasketBingo) {
      ...
      if(isBingo()) {
        alert(`${namePlayer} has conseguido Bingo en el turno número ${counterRounds}`);
        if(counterRounds <= 75) {
          players.push({name: namePlayer[0], puntuacion: points[0]});
          alert(`Te voy a dar ${points[0]} ISDINES, `);
          return;
        } else if (counterRounds < 79) {
          players.push({name: namePlayer[0], puntuacion: points[1]});
          alert(`Te voy a dar ${points[1]} ISDINES`);
          return;
        } else if (counterRounds < 83) {
          players.push({name: namePlayer[0], puntuacion: points[2]});
          alert(`Ahhh, pero tenés mucha mala suerte. Te corresponden ${points[2]} ISDINES`);  
          return;
        } else if (counterRounds >= 83) {
          players.push({name: namePlayer[0], puntuacion: points[4]});
          alert(`El haber excedido el turno 82 no solo no puntúa, sino que castiga 👊; copia la siguiente dirección y visualiza integramente el siguiente video: ${points[3]}`);
          return;
        };
      };
    };
  ...
  players.sort((a, b) => (a.puntuacion > b.puntuacion) ? 1 : (a.puntuacion === b.puntuacion) ? ((a.name.toLowerCase() > b.name.toLowerCase()) ? 1 : -1) : -1)
  ...
  playAgain = () => {
    let repeatGame = prompt(`${namePlayer}, ¿te gustaría volver a jugar? (si/no)`);
    if (repeatGame === "si" || repeatGame === "Si" || repeatGame === "sí" || repeatGame === "Sí") {
    ...
    } else if (repeatGame === "no") {
      return console.log(console.log(`%cGracias ` + namePlayer + `, nos vemos pronto.`, "color:yellow; background-color:blue"));
    } else if (Number(repeatGame) || repeatGame === "" || repeatGame === " " || repeatGame === "null" || repeatGame == null) {
  ...
  console.log(`⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩ EL RANKING ⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩⇩`)
  orderRanking();
  if (counterRounds >= 83) {
    console.log(`No olvides que tienes una prenda que cumplir: https://www.youtube.com/watch?v=x8zFL-0rBAw&ab_channel=DiditDeelon`)
  }
  playAgain();
