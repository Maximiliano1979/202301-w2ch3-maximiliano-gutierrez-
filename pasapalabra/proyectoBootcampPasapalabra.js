...
const getRules = () => {
  ...
  alert(rules);
  console.log(`%cLAS REGLAS DEL JUEGO:
_____________________________________________________________________________________________________________________________________________________________________________________
${rules}
_____________________________________________________________________________________________________________________________________________________________________________________`, "color:black; background-color:white");
};


const game = () => {
  let minutesFromStart = new Date().getMinutes();
  do {
    questions1.forEach(function (pregunta) {
      if (endGame === false) {
      if (pregunta.status === 0) {
        ...
        } else if (answerUser.toLowerCase() === pregunta.answer) { // REFACTORIZAR CON SWITCH
          pregunta.status = 1;
          counterAnswer++;
          alert(`Respuesta correcta. ${userName}, en este momento tienes estos puntos: ${counterAnswer}`);
        } else if (answerUser.toLowerCase() === "pasapalabra") {
          pregunta.status = 0;
        } else if (answerUser.toLowerCase() === "end") {
          questions1.forEach(function (element) {
          element.status = 1;
          });
  ...
  } while (questions1.some((element) => element.status === 0) && endGame === false);
};
...
const playAgain = () => {
  let repeatGame = prompt(`${userName}, ¿te gustaría volver a jugar? (si/no)`);
  if (repeatGame === "si" || repeatGame === "Si" || repeatGame === "sí" || repeatGame === "Sí") {
    ...
    return console.log(console.log(`%cGracias ` + userName + `, nos vemos pronto.`, "color:yellow; background-color:blue"));
  } else if (Number(repeatGame) || repeatGame === "" || repeatGame === " " || repeatGame === null) {
...
const pasapalabraApp = () => {
  const userName = askForNameAndGreet();
  getRules();
  game();
  orderRanking();
  playAgain();
